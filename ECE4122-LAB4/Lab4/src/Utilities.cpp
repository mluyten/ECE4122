...
Class: ECE4122
Last Date Modified: 11/5/2024

Description:
This implements utility functions. In this case, it only implements
...
// -t <val> is stored at map["memoryMode"]
		...
		if (arg == "-n") {
            ...
            if (i + 1 < argc) {
                try {
                    ...
                    std::cerr << "Error: Invalid value for -t (Must be NORMAL, PINNED, or MANAGED): " << argv[i + 1] << std::endl;
					args["error"] = 1;
                }
            } 
            else {
